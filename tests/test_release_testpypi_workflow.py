...
class ReleaseTestPyPIWorkflowTests(unittest.TestCase):
    def test_release_testpypi_workflow_exists(self) -> None:
        ...
        self.assertIn("Publish to TestPyPI with API token", content)
        ...
        self.assertIn("password: ${{ env.TEST_PYPI_API_TOKEN }}", content)
        self.assertIn("attestations: false", content)
